...
void HAL_SPI_MspInit(SPI_HandleTypeDef* spiHandle)
{
 ...
 /*Configure GPIO pin Output Level CS for SD card  */
...
void SPI_SendMulti(SPI_TypeDef* SPIx, uint8_t* dataOut, uint8_t* dataIn, uint32_t count) {
	...
	uint32_t start_timer; //timeout
	start_timer = HAL_GetTick();  //timeout

	while (count--) {
		/* Wait busy */
		while (((SPIx->SR & SPI_FLAG_TXE) == 0 || (SPIx->SR & SPI_FLAG_BSY)) && ((HAL_GetTick() - start_timer) < 500));  //+timeout 500ms
		
		/* Fill output buffer with data */
		*(__IO uint8_t *)&SPIx->DR = *dataOut++;
		
		/* Wait for SPI to end everything */
		while (((SPIx->SR & SPI_FLAG_RXNE) == 0 || (SPIx->SR & SPI_FLAG_BSY)) && ((HAL_GetTick() - start_timer) < 500)); //+timeout 500ms
