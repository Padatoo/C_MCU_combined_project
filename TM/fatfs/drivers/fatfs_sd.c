...
)
{
	...
	SPI_WriteMulti(FATFS_SPI, buff, btx);
...
)
{
  ...
  uint32_t start_timer;
	...
	INSIDE WHILE WE DO THE FOLLOWING CHECK
	if ((((HAL_GetTick() - start_timer) >=  Timeout) && (Timeout != HAL_MAX_DELAY)) || (Timeout == 0U))
        {
	...
	*/
	...
	} while (((d != 0xFF ) && ((HAL_GetTick() - start_timer) < wt)));	/* Wait for card goes ready or timeout */
...
static int rcvr_datablock (	/* 1:OK, 0:Error */
	...
	*/
	...
	} while ((token == 0xFF) && ((HAL_GetTick() - start_timer) < 200));
	if (token != 0xFE) {
	...
	if (send_cmd(CMD0, 0) == 1) {				/* Put the card SPI/Idle state */
		...
		if (send_cmd(CMD8, 0x1AA) == 1) {	/* SDv2? */
			...
			if (ocr[2] == 0x01 && ocr[3] == 0xAA) {				/* Is the card supports vcc of 2.7-3.6V? */
				while ((((HAL_GetTick() - start_timer) < 500)) && send_cmd(ACMD41, 1UL << 30)) ;	/* Wait for end of initialization with ACMD41(HCS) */
			...
			while (((HAL_GetTick() - start_timer) < 500) &&/*TM_DELAY_Time2() &&*/ send_cmd(cmd, 0));			/* Wait for end of initialization */
